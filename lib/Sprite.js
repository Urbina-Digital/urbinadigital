...
class Sprite {
  constructor({ context, image, position, scale = 1, framesMax = 1, frameOrientation = 'vertical', offset = {x: 0, y: 0}, direction = 'right', noRepeat = true}) {
  ...
  draw() {
    ...
    const frameWidth = this.frameOrientation === 'vertical' ? this.image.width : (this.image.width / this.framesMax);
    const frameHeight = this.frameOrientation === 'vertical' ? (this.image.height / this.framesMax) : this.image.height;
    const drawWidth = this.frameOrientation === 'vertical' ? (this.image.width * this.scale) : (this.image.width / this.framesMax) * this.scale;
    const drawHeight = this.frameOrientation === 'vertical' ? ((this.image.height / this.framesMax) * this.scale) : this.image.height * this.scale;
  ...
  drawFlipped(frameWidth, frameHeight, drawWidth, drawHeight) {
    const sourceX = this.frameOrientation === 'vertical' ? 0 : this.framesCurrent * frameWidth;
    const sourceY = this.frameOrientation === 'vertical' ? this.framesCurrent * frameHeight : 0;
...
class Avatar extends Sprite {
  ...
  update() {
    ...
    //gravity function
    if ((this.position.y + this.height + this.velocity.y) >= this.context.canvas.height - 16) {
      this.velocity.y = 0;
      this.position.y = this.context.canvas.height - this.height - 16; // setting 'bottom' threshold for background
